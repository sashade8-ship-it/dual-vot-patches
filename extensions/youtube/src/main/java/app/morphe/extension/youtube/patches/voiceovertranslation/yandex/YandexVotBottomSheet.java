...
public class YandexVotBottomSheet {
    ...
    public static void show(Context context) {
        try {
            ...
            YandexVoiceOverTranslationPatch.setOnTranslationStateChangeCallback(YandexVotBottomSheet::refreshStatusIfVisible);
    ...
    private static void updateStatusText(TextView statusText) {
        if (YandexVoiceOverTranslationPatch.translationStarting) {
            statusText.setText(str("dualvot_yandex_stream_waiting"));
    ...
    private static LinearLayout createVoiceStyleButtons(Context context) {
        ...
        Runnable updateSelection = () -> {
            boolean useLive = Settings.DUAL_VOT_YANDEX_USE_LIVE_VOICES.get();
        ...
        standardButton.setOnClickListener(v -> {
            Settings.DUAL_VOT_YANDEX_USE_LIVE_VOICES.save(false);
            updateSelection.run();
            YandexVoiceOverTranslationPatch.restartTranslationIfActive();
        });
        liveButton.setOnClickListener(v -> {
            Settings.DUAL_VOT_YANDEX_USE_LIVE_VOICES.save(true);
            updateSelection.run();
            YandexVoiceOverTranslationPatch.restartTranslationIfActive();
        });
